...
def video_play():
    ...
    if ret:
        ...
        # 녹색 필터 마스크
        frame_g = cv2.inRange(frame, (36, 25, 25), (70, 255, 255))
        ...
        # np.add를 통해 적,녹 비디오 합침. 최종 출력을 위해 frame 변수에 저장
        ...
        # im_with_keypoints_r = cv2.drawKeypoints(video_red, keypoints_r, np.array([]), (0,0,2500), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        # im_with_keypoints_g = cv2.drawKeypoints(video_green, keypoints_g, np.array([]), (0,2500,0), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)

        # drawKeypoints(입력영상, 키포인트, 출력영상) : 화면에 인식한 레이저 (키포인트)를 표시해줌
        im_with_keypoints_r = cv2.drawKeypoints(video_red, keypoints_r, frame, (0, 0, 2500),
                                                cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        im_with_keypoints_g = cv2.drawKeypoints(video_green, keypoints_g, frame, (0, 2500, 0),
                                                cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        ...
        ### 비디오 출력 화면 지정 (출력원하는 img변수 주석 풀고 사용하면 됨)
        # 1. 빨 + 초 같이
