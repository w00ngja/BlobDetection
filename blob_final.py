...
def video_play():
    ...
    if ret:
        ...
        # 녹색 필터 마스크 (수비 Lazer)
        frame_g = cv2.inRange(frame_hsv, (-10, 100, 100), (10, 255, 255))
        ...
        # 불 키고 한다는 가정 하에 십자형 골대는 흑백처리하면 될 듯?
        ...
        for point in keypoints_g:
            lz_x_g = point.pt[0]
            lz_y_g = point.pt[1]
            lazer_circle_x.set("lazer_x (green) : " + str(int(lz_x_g)))
            lazer_circle_y.set("lazer_y (green) : " + str(int(lz_y_g)))
